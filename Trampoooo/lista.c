...
RemList *criar_lista(FILE *f, int t){
	...
	if(prox != -1){
		while(prox != -1 && (feof(f) == 0)){	
			...
			if (t=1)
				insercOrdCresc(n, offset, tamanho, prox);
			if (t=2)
				insercOrdDec(n, offset, tamanho, prox);
			n->cont++;
...
void print_lista(RemList* l){
	...
	while(aux != NULL){
		printf(" %-ld| %-d| %-ld| -->", aux->offset, aux->tamanho, aux->prox);
		aux = aux->proximo;
