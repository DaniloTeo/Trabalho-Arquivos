...
void firstFit(Group *g, Registro_Delimitador *r){
	...
	if(aux == NULL){//se nao ha espacos colocar no fim do arquivo
		...
		*/	
		...
		aux->tamanho = aux->tamanho - tam_r - 1;
		RemListToFile(g, rem);
	}
	...
	g->array[g->length - 1].offset = aux->offset + aux->tamanho + 1;
	qsort(g->array, g->length, sizeof(ChPrim), compare); // ordena o array
}
...
void bestFit(Group *g, Registro_Delimitador *r){
	...
	if(aux == NULL){//se nao ha espacos colocar no fim do arquivo
		...
		*/	
		...
		//atualizando o novo tamanho do espaco restante
		aux->tamanho = aux->tamanho - tam_r - 1;
		ordenar_lista(rem, 1);//ordenando espacos removidos de crescente de acordo com o tamanho
		RemListToFile(g, rem);
	}
...
void worstFit(Group *g, Registro_Delimitador *r){
	...
	if(aux->tamanho < tam_r + 13) aux == NULL;
	...
	if(aux == NULL){//se nao ha espacos colocar no fim do arquivo
		...
		*/	
		...
		//atualizando o novo tamanho do espaco restante
		aux->tamanho = aux->tamanho - tam_r - 1;
		ordenar_lista(rem, 2);//ordenando espacos removidos de crescente de acordo com o tamanho
		RemListToFile(g, rem);
	}
...
/*
void exemploInsercao (Group* g, Registro_Delimitador* r) {
	
	RemList *rem = criar_lista(g->file);
	int tam_r = sizeReg(r);

	// Pega primeiro Node que: Node->tamanho > tam_r (nao precisa de + 13 aqui)
	// O jeito de pegar o primeiro muda bastante dependendo da implementacao:
	// worst fit -- pega o primeiro da lista e verifica se ele cabe.
	// best fit -- percorre a lista ate encontrar o primeiro que cabe (obs: lista esta ordenada)
	// first fit -- percorre a lista ate encontrar o primeiro que cabe (obs: lista nao esta ordenada)
	// Se não encontrar nenhum, então escreve novo registro no final e dá return.

	//Continua rotina, se encontrou...

	// Escreve o novo registro
	long int new_reg_offset = Node->offset + (Node->tamanho - tam_r + 1);
	fseek(g->file, new_reg_offset, SEEK_SET);
	putc('~', g->file);
	WriteReg_Delimitador(r, g->file);//escrevendo no registro

	// remove da lista o Node, se ele nao ocupa mais espaco o bastante
	if (Node->tamanho < tam_r + 13){
		//remove Node da lista (muito cuidado com sua função de remocao)
	}
	else { // mas se ele ainda tiver espaço o bastante, deve ser atualizado
		//atualiza tamanho do node (NAO PRECISA ATUALIZAR EM DISCO!!)
		Node->tamanho = Node->tamanho + tam_r 1
	}

	// Atualiza a lista em arquivo com a lista ligada em RAM
	RemListToFile(g, rem);
